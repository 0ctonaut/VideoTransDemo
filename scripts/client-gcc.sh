#!/bin/bash
# GCC WebRTC Client script
#
# Usage:
#   ./scripts/client-gcc.sh --video assets/Ultra.mp4 [--session NAME] [--ip 192.168.100.2]
#                           [--max-duration 60s] [--max-size 200]
#
# 说明：
#   - 自动发现或创建 GCC 实验 session 目录
#   - 通过 offer/answer 文件与 server-gcc 完成 SDP 握手
#   - 运行 client-gcc 接收 H.264 原始码流
...
# Session directory: 如果未指定，则选择最新的 session_gcc_* 目录
if [ -z "$SESSION_NAME" ]; then
    LATEST_SESSION=$(ls -td session_gcc_* 2>/dev/null | head -1 || true)
    if [ -z "$LATEST_SESSION" ]; then
        echo "Error: No session_gcc_* directory found. Please run server-gcc.sh first or specify --session."
        exit 1
    fi
...
if [ ! -f "$OFFER_FILE" ]; then
    ...
    echo "Make sure server-gcc.sh has been started and generated the offer."
    exit 1
fi

# Build GCC client if needed
CLIENT_BIN="./build/client-gcc"
if [ ! -f "$CLIENT_BIN" ] || [ "src/client-gcc.go" -nt "$CLIENT_BIN" ] || [ "src/common.go" -nt "$CLIENT_BIN" ] || [ "src/metrics.go" -nt "$CLIENT_BIN" ]; then
    echo "Building GCC client..."
    make client-gcc
fi

echo "=========================================="
echo "  GCC WebRTC Client Session"
echo "=========================================="
