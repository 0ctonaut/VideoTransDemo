#!/bin/bash
# NDTC WebRTC Client script
#
# Usage:
#   ./scripts/client-ndtc.sh --video assets/Ultra.mp4 [--session NAME] [--ip 192.168.100.2]
#                            [--max-duration 60s] [--max-size 200]
#
# 说明：
#   - 自动发现或指定 NDTC 实验 session 目录
#   - 通过 offer/answer 文件与 server-ndtc 完成 SDP 握手
#   - 运行 client-ndtc 接收 H.264 原始码流
...
# Session directory: 如果未指定，则选择最新的 session_ndtc_* 目录
if [ -z "$SESSION_NAME" ]; then
    LATEST_SESSION=$(ls -td session_ndtc_* 2>/dev/null | head -1 || true)
    if [ -z "$LATEST_SESSION" ]; then
        echo "Error: No session_ndtc_* directory found. Please run server-ndtc.sh first or specify --session."
        exit 1
    fi
...
if [ ! -f "$OFFER_FILE" ]; then
    ...
    echo "Make sure server-ndtc.sh has been started and generated the offer."
    exit 1
fi

# Build NDTC client if needed
CLIENT_BIN="./build/client-ndtc"
if [ ! -f "$CLIENT_BIN" ] || [ "src/client_ndtc.go" -nt "$CLIENT_BIN" ] || [ "src/common.go" -nt "$CLIENT_BIN" ]; then
...
echo "=========================================="
echo "  NDTC WebRTC Client Session"
echo "=========================================="
