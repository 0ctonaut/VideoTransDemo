...
# Build Salsify client if needed（直接用 go build，避免依赖 Makefile 中的额外修改）
CLIENT_BIN="./build/client-salsify"
if [ ! -f "$CLIENT_BIN" ] || [ "src/client_salsify.go" -nt "$CLIENT_BIN" ] || [ "src/common.go" -nt "$CLIENT_BIN" ]; then
    echo "Building Salsify client..."
    mkdir -p build
    go build -v -tags salsify -o "$CLIENT_BIN" src/client_salsify.go src/common.go src/metrics.go src/salsify_controller.go src/h264_writer.go
fi
