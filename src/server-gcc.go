...
// server-gcc.go - GCC 实验用 WebRTC 服务器
//
// 说明：
//   - 基本上复用现有 server.go 的逻辑：从本地视频文件读取 → FFmpeg 解码+重编码 H.264 → 通过 Pion 发送。
//   - 主要差异：
//   - 支持通过 -session-dir 指定当前实验的 session 目录（与脚本配合）
//   - 保持代码结构简单，让 GCC / Metrics 实验集中在 client-gcc 侧和后续扩展中完成。
package main
